...
Plugin.register = async function(app, plugins) {
    ...
    const remove = (await Plugin.findAll({where: {id: {[SQ.Op.notIn]: plugins} }})).map(d => d.id);
    const keep = await PluginData.findAll({where: {plugin_id: {[SQ.Op.in]: remove}, key: 'installed_by'}});
    keep.forEach(p => {
